-- FIX COMPLIANCE DEPENDENCIES V6 (FINAL + DATES FIXED)
-- Run this entire script in Supabase SQL Editor
-- Fixes: "column deadline_date does not exist" and timestamp/date casting issues
...
SELECT 
  ...
  -- Compliance placeholders (base view doesn't have them yet)
...
-- Ensure all compliance columns exist (especially deadline_date)
DO $$
BEGIN
    ...
    END IF;
    -- THE CRITICAL FIX: Ensure deadline_date exists
...
-- Robust Property Seeding (checks for appfolio_property_id)
DO $$
BEGIN
...
-- Seed Data for Compliance (with DATE casting fix)
