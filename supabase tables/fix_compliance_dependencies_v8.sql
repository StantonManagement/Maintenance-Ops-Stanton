-- FIX COMPLIANCE DEPENDENCIES V8 (CORRECT COLUMN TYPE)
-- Run this entire script in Supabase SQL Editor
-- Fixes: "invalid input syntax for type integer: S0021" by fixing the column type
...
-- ==========================================
-- 2. TABLE: PROPERTIES (Fix the schema)
-- ==========================================
...
-- CRITICAL FIX: Convert appfolio_property_id to TEXT if it exists as integer
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'properties' AND column_name = 'appfolio_property_id') THEN
        -- Alter column to TEXT to support values like 'S0021'
...
SELECT 
  ...
  NOW() as updated_at
FROM properties p
-- Ensure code is treated as text for the join
...
-- ==========================================
-- 6. PERMISSIONS & SEEDING (Using correct TEXT types)
-- ==========================================
...
-- Seeding: Now safe to use strings for appfolio_property_id
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'properties' AND column_name = 'appfolio_property_id') THEN
