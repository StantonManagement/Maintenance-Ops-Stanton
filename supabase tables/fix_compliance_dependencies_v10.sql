-- FIX COMPLIANCE DEPENDENCIES V10 (DROP CONSTRAINTS)
-- Run this entire script in Supabase SQL Editor
-- Fixes: "violates check constraint compliance_deadlines_status_check" by removing the legacy constraint
...
BEGIN
    ...
    -- Fix appfolio_property_id type if it exists
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'properties' AND column_name = 'appfolio_property_id') THEN
        ALTER TABLE properties ALTER COLUMN appfolio_property_id TYPE TEXT USING appfolio_property_id::text;
    END IF;
END $$;
...
-- CRITICAL FIX: DROP THE RESTRICTIVE CONSTRAINT
ALTER TABLE compliance_deadlines DROP CONSTRAINT IF EXISTS compliance_deadlines_status_check;

-- Ensure Columns
DO $$
BEGIN
    -- Relax legacy columns if they exist
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'compliance_deadlines' AND column_name = 'type') THEN
        ALTER TABLE compliance_deadlines ALTER COLUMN type DROP NOT NULL;
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'compliance_deadlines' AND column_name = 'deadline') THEN
        ALTER TABLE compliance_deadlines ALTER COLUMN deadline DROP NOT NULL;
    END IF;
